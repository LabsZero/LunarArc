...
public class CraftItemMeta implements ItemMeta, org.bukkit.inventory.meta.Damageable, org.bukkit.inventory.meta.Repairable {
    ...
    /** Unknown/unsupported native 1.21.1 component patch entries. Keeping the
     * Optional preserves explicit removals as well as values, matching Paper's
     * unhandled/removed component behaviour instead of silently discarding modded
     * or future-compatible item data when a plugin round-trips ItemMeta. */
    private Map<net.minecraft.core.component.DataComponentType<?>, Optional<?>> unhandledDataComponents = new LinkedHashMap<>();
    private int version = 0;
    private org.bukkit.craftbukkit.v1_21_R1.persistence.CraftPersistentDataContainer persistentDataContainer =
            new org.bukkit.craftbukkit.v1_21_R1.persistence.CraftPersistentDataContainer();

    public CraftItemMeta() {}

    /**
     * Convert an arbitrary Bukkit ItemMeta into LunarArc's concrete metadata model.
     * Paper's ItemStack delegates always receive server-owned CraftMetaItem instances,
     * but plugins are allowed to pass API-compatible metadata objects back to
     * ItemStack#setItemMeta.  Copy the public Paper 1.21.1 surface instead of
     * dropping metadata during Bukkit -> NMS conversion.
     */
    public static @NotNull CraftItemMeta copyOf(@NotNull ItemMeta source) {
        ...
        // Paper adventure-mode predicates.  These methods are part of the pinned
        // 1.21.1 ItemMeta contract and are important for adventure maps/tools.
        copy.setCanDestroy(source.getCanDestroy());
        copy.setCanPlaceOn(source.getCanPlaceOn());

        // Most metadata objects reaching this path are LunarArc/Craft objects. If a
        // plugin supplies a foreign implementation, the public values above are still
        // preserved. PDC is copied when the implementation exposes our concrete store.
        if (source.getPersistentDataContainer() instanceof org.bukkit.craftbukkit.v1_21_R1.persistence.CraftPersistentDataContainer pdc) {
            copy.persistentDataContainer = new org.bukkit.craftbukkit.v1_21_R1.persistence.CraftPersistentDataContainer(pdc);
        }
        return copy;
    }
    ...
    public CraftItemMeta(ItemStack nms) {
        ...
        // Read item name independently from the editable custom/display name.
    ...
    public void applyToNms(ItemStack nms) {
        ...
        // Item name (distinct from custom/display name)
        if (itemName != null) {
            try {
    ...
    /** Apply the 1.21.1 ENCHANTMENTS data component from Bukkit meta. */
    private void applyEnchantments(ItemStack nms) {
        try {
            ...
            Object minecraftServer = ((org.bukkit.craftbukkit.v1_21_R1.CraftServer) org.bukkit.Bukkit.getServer()).getServer();
    ...
    /** Build the native 1.21.1 ATTRIBUTES data component from Bukkit modifiers. */
    private void applyAttributeModifiers(ItemStack nms) {
        ...
        try {
            ...
            for (Map.Entry<Attribute, AttributeModifier> entry : attributeModifiers.entries()) {
                ...
                Object nmsModifier = org.bukkit.craftbukkit.v1_21_R1.attribute.CraftAttributeInstance.toMinecraft(entry.getValue());
    ...
    public @NotNull org.bukkit.inventory.meta.tags.CustomItemTagContainer getCustomTagContainer() {
        return new org.bukkit.craftbukkit.v1_21_R1.inventory.tags.DeprecatedCustomTagContainer(this.persistentDataContainer);
    }
    ...
    public @NotNull String getAsString() {
        try {
            ...
            net.minecraft.nbt.Tag nbt = net.minecraft.core.component.DataComponentPatch.CODEC
                    .encodeStart(net.minecraft.server.MinecraftServer.getDefaultRegistryAccess()
    ...
    public @NotNull String getAsComponentString() {
        try {
            ...
            applyToNms(stack);
            net.minecraft.core.component.DataComponentPatch patch = stack.getComponentsPatch();
            net.minecraft.core.RegistryAccess registryAccess = net.minecraft.server.MinecraftServer.getDefaultRegistryAccess();
    ...
    @Override public @NotNull org.bukkit.inventory.meta.components.ToolComponent getTool() {
        ...
        // Paper exposes a mutable snapshot even when no component is currently set.
    ...
    @Override public @NotNull org.bukkit.inventory.meta.components.FoodComponent getFood() {
        ...
        return new CraftFoodComponent(new net.minecraft.world.food.FoodProperties(0, 0.0f, false, net.minecraft.world.food.FoodProperties.DEFAULT_EAT_SECONDS, java.util.Optional.empty(), java.util.List.of()));
    }
    ...
    public @NotNull CraftItemMeta clone() {
        try {
            ...
            clone.persistentDataContainer =
                    new org.bukkit.craftbukkit.v1_21_R1.persistence.CraftPersistentDataContainer(persistentDataContainer);
            return clone;
