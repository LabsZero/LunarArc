 ...
 *
 * Slot layout (mirrors Bukkit convention):
 *  0-35  main inventory (0-8 hotbar, 9-35 storage)
 *  36    boots
 *  37    leggings
 *  38    chestplate
 *  39    helmet
...
public class CraftPlayerInventory implements PlayerInventory, org.bukkit.inventory.EntityEquipment {
    ...
    // -----------------------------------------------------------------------
    // NMS <-> Bukkit slot mapping
    // -----------------------------------------------------------------------

    private net.minecraft.world.item.ItemStack nms(int slot) {
        if (slot < 36) return handle.items.get(slot);
        if (slot == 36) return handle.armor.get(0); // boots
        if (slot == 37) return handle.armor.get(1); // leggings
        if (slot == 38) return handle.armor.get(2); // chestplate
        if (slot == 39) return handle.armor.get(3); // helmet
